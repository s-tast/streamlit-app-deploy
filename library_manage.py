...
def list_member_borrowed_books(member_id):
    """
    指定した会員が現在借りている図書の一覧を表示する関数
    """
    found = False
    print(f"--- 会員ID「{member_id}」が借りている図書一覧 ---")
    for record in borrow_records:
        if record["member_id"] == member_id and not record["returned"]:
            book = next((b for b in books if b["book_id"] == record["book_id"]), None)
            if book:
                print(f"図書: {book['title']}（ID: {book['book_id']}）, 貸出日: {record['borrow_date']}, 返却期限: {record['due_date']}")
                found = True
    if not found:
        print("現在、この会員が借りている図書はありません。")
